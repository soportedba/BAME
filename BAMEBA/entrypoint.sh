...
function setup_database() {
  ...
  if ! psql -lqt -U $DB_USER -h $DB_HOST | grep -w hibernate; then
    ...
    psql -U $DB_USER -h $DB_HOST -p $DB_PORT -f $PENTAHO_HOME/biserver-ce/data/postgresql/create_jcr_postgresql.sql
    psql -U $DB_USER -h $DB_HOST -p $DB_PORT -f $PENTAHO_HOME/biserver-ce/data/postgresql/create_quartz_postgresql.sql
    psql -U $DB_USER -h $DB_HOST -p $DB_PORT -f $PENTAHO_HOME/biserver-ce/data/postgresql/create_repository_postgresql.sql
...
if [ "$1" = 'run' ]; then
  ...
  nohup bash -c "$PENTAHO_HOME/biserver-ce/data/start_hypersonic.sh" &
